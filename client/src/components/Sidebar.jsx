...
const Sidebar = ({isMenuOpen , setIsMenuOpen}) => {
  ...
  return (
    <div className= {`flex flex-col h-screen min-w-72 p-5 dark:bg-orang border-r border-[#80609F]/30 backdrop-blur-3xl
    transistion-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
    ...
    text-white bg-gradient-to-r from-[A456F7] to-[#3D8156] text-sm rounded-md
    cursor-pointer'>
    ...
    <div className='flex-1 overflow-x-scroll mt-3 text-sm space-y-3'>
    
       {
        ...
        includes(search.toLowerCase())).map((chat) =>(
          ...
          {chat.messages.length > 0 ? chat.messages[0].content.slice(0,32): chat.name} </p>
    ...
    <div className='flex flex-col text-sm'>
      ...
      <p className='text-xs text-gray-400'> Purchase credits for the quickgpt </p>
    </div>
